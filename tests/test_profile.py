...
PROFILE = """
name: test
vision: {camera: 0, width: 320, height: 180, fps: 20, min_confidence: 0.5, gesture: wrist_buttons, gesture_hold_ms: 400, gesture_menu_hold_ms: 900, gesture_raise_margin: 0.08, gesture_stale_after_ms: 250}
...
uinput: {x_axis: ABS_X, y_axis: ABS_Y, accept_button: BTN_SOUTH, decline_button: BTN_WEST, menu_button: BTN_START}
"""
...
class ProfileValidationTests(unittest.TestCase):
    ...
    def test_loads_wrist_gesture_and_accept_button(self):
        ...
        self.assertEqual(profile.vision.gesture, "wrist_buttons")
        self.assertEqual(profile.vision.gesture_hold_ms, 400)
        self.assertEqual(profile.vision.gesture_menu_hold_ms, 900)
        self.assertEqual(profile.uinput.accept_button, "BTN_SOUTH")
        self.assertEqual(profile.uinput.decline_button, "BTN_WEST")
        self.assertEqual(profile.uinput.menu_button, "BTN_START")

    def test_rejects_unknown_gesture(self):
        with TemporaryDirectory() as directory:
            path = Path(directory) / "test.yaml"
            source = PROFILE % ("speed_kph", 0, 40, 1)
            path.write_text(source.replace("wrist_buttons", "wave"), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "Unknown vision.gesture 'wave'"):
                load_profile(str(path))

    def test_rejects_duplicate_buttons(self):
        with TemporaryDirectory() as directory:
            path = Path(directory) / "test.yaml"
            source = PROFILE % ("speed_kph", 0, 40, 1)
            path.write_text(source.replace("menu_button: BTN_START", "menu_button: BTN_SOUTH"), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "buttons must be different"):
                load_profile(str(path))
